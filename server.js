...
const STATIC = {
  ...
  terms: {
    ...
    signatureLabel: 'Yπογραφή / Σφραγίδα',
    dateLabel: '',
    website: 'www.omprela.gr',
    email: 'e-mail: info@omprela.gr',
    phone1: '23214.01146',
...
// ── Helper: build render data from session ────────────────────────────────────
function buildRenderData(type, data) {
      ...
      image_src: p.image_src
        ? (p.image_src.startsWith('http')
            ...
            : p.image_src.replace(/^\//, '').startsWith('products/')
              ? `${S3_BASE}/${p.image_src.replace(/^\//, '')}`
              : `http://localhost/item_images/${p.image_src.replace(/^.*item_images\//, '')}`)
        : null,
    })),
